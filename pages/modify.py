...
from .functions import check_float, id_search, Students_data, add_stud_file, remove_student
...
class Modify(ctk.CTkFrame):  
    def __init__(self, master):
        ...
        labels = ["  ID :", "  1st name :", "  2nd name :", "  Phone number :", "  CIN :", "  Sexe :"]
    ...
    def get_entries(self):
        ...
        if not name2:
            ...
            return None
        phone = check_float(self.entries[3].get())
        if phone == -1 or phone < 99999999:
            self.incorr.configure(text="Phone number incorrect!")
            return None
        cin = self.entries[4].get().strip()
        if not cin or len(cin) < 8:
            self.incorr.configure(text="CIN incorrect!")
            return None
